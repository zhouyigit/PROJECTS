...
- (BOOL)checkDatabase:(NSString*)dbFullName newVersion:(int)newVersion
{
    ...
    @try {
        ...
        if (oldVersion <= 0) { //表示第一次创建数据库
            ...
            BOOL rev = [self onCreate:_db];//第一次建表方法
            if (rev) {
...
//                    @throw [NSException exceptionWithName:@"Database version error" reason:errStr userInfo:nil];
                ...
                //执行用户的代码
                [_db beginTransaction];
                BOOL rev = [self onEqual:self.db oldVersion:oldVersion newVersion:newVersion];//数据库版本号没变时，执行的方法
                if (rev)
                    [_db commit];
                else {
                    [_db rollback];
                ...
                //执行用户的更新代码
                [_db beginTransaction];
                BOOL rev = [self onUpgrade:self.db oldVersion:oldVersion newVersion:newVersion];//数据库升级方法
                if (rev) {
                    rev = [_db executeUpdate:[NSString stringWithFormat:@"PRAGMA user_version = %d", newVersion]];
                    if (rev)
                        [_db commit];
